...
export default class Scenicspotproductlist extends Component{
	constructor(props){
	   super(props); 
	...
	componentDidMount() {
	   this.props.loadScenicspotArticles();
	}
	...
	render() {
	  ...
	  return ( 
		  ...
		  <Travellist productName="景点" productPic={<img src={require("../../theme/image/jingdian.png")}/>}> 
             <Tabs defaultActiveKey={1} id="uncontrolled-tab-example1" bsStyle="pills">
						<Tab eventKey={1} title="主推">
					      <div style={{borderTop:'1px solid red',marginTop:'10px',position:'relative'}}>
		                   <div style={{position:'absolute',right:'0px',top:'-20px',cursor:'pointer'}}>
		                     <Link to="/scenicspot/?select=zhutui">更多产品></Link>
		                   </div>
						...
						<Tab eventKey={2} title="热销">
						  <div style={{borderTop:'1px solid red',marginTop:'10px',position:'relative'}}>
		                   <div style={{position:'absolute',right:'0px',top:'-20px',cursor:'pointer'}}>
		                    <a onClick={this.handleNavigate.bind(this,"/scenicspot","rexiao")}>更多产品></a>
		                   </div>
						   <div className={Styles.allItem}>
...
Scenicspotproductlist.propTypes={
